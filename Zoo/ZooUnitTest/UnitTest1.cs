...
namespace ZooUnitTest
{
...

	{
		...
		[Fact]
		public void TestCatVaildForAdoption()
		{
			Cat cat =new Cat("Lolo","4","Meat","Hair");
			cat.Adoption = false;
			bool TestAdoption = cat.VaildForAdoption();
			Assert.Equal(false, TestAdoption);
		}

		[Fact]
		public void TestCanaryForSafe()
		{
			Canary canary = new Canary("Lolo", "2", "Meat");
			canary.Adoption = false;
			bool TestSafe = canary.Safe();
			Assert.Equal(true, TestSafe);
		}

		[Fact]
		public void TestCanaryLivesWithHuman()
		{
			Canary canary = new Canary("Lolo", "2", "Meat");
			canary.Adoption = true;
			bool TestLivesWithHuman = canary.LivesWithHuman();
			Assert.Equal(true, TestLivesWithHuman);
		}

		[Fact]
		public void TestLionForSafe()
		{
			Lion lion = new Lion("Lolo", "4", "Meat","Hair");
			
			bool TestSafe = lion.Safe();
			Assert.Equal(false, TestSafe);
		}

		[Fact]
		public void TestLionLivesWithHuman()
		{
			Lion canary = new Lion("Lolo", "4", "Meat", "Hair");
			
			bool TestLivesWithHuman = canary.LivesWithHuman();
			Assert.Equal(false, TestLivesWithHuman);
		}
